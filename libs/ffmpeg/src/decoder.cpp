...
// Pre-load the C++ stdlib headers we use so their declarations are
// established with C++ linkage *before* the extern "C" block below.
// Inside the extern "C" block, FFmpeg's transitive `<math.h>` /
// `<stddef.h>` includes then hit already-set include guards and don't
// re-introduce libstdc++'s C++ types under C linkage.
#include <algorithm>
#include <cmath>
#include <cstddef>
#include <cstdint>
...
// FFmpeg public headers are plain C (no internal `extern "C"`), so we
// must wrap them ourselves.
extern "C" {
...
module waywallen.ffmpeg;

import rstd;

namespace waywallen::ff {
namespace {
